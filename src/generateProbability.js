...
const probabilityTitle = document.querySelector('.probability-output--title');
...
const renderProbability = (result) => {
  ...
  probabilityTitle.innerHTML = `Stick and Switch probabilities for ${result.totalDoors} doors when host opens ${result.opened} doors`;
...
export const showDefaultProbability = () => {
  doorsInput.value = 3;
  openDoorsInput.value = 1;
  let result = calculatePobability(doorsInput.value, openDoorsInput.value);
  renderProbability(result);
};
